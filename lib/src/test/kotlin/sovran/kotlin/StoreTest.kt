...
//        store.reset()
    ...
    fun testDoubleSubscribe() = runBlocking {
        ...
        val id1 = store.subscribe(this@StoreTest, MessagesState::class) { state ->
            print("unreadCount = $state.unreadCount")
        }

        // subscribe self to UserState twice.
        store.subscribe(this@StoreTest, UserState::class) { state ->
            print("username = $state.username")
        }

        // this should add a second listener for UserState
        val id3 = store.subscribe(this@StoreTest, UserState::class) { state ->
    ...
    fun testDispatch() = runBlocking {
        ...
        // register some handlers for state changes
        store.subscribe(subscriber = this@StoreTest, stateClazz = MessagesState::class) { state ->
            latch.countDown()
    ...
    fun testAsyncDispatch() = runBlocking {
        ...
        // register some handlers for state changes
        store.subscribe(subscriber = this@StoreTest, initialState = false, stateClazz = MessagesState::class) { state ->
            latch.countDown()
    ...
    fun testDroppedAsyncDispatch() = runBlocking {
        ...
        // register some handlers for state changes
        store.subscribe(subscriber = this@StoreTest, initialState = true, stateClazz = MessagesState::class) { state ->
            latch.countDown()
    ...
    fun testUnprovidedStateAsyncDispatch() = runBlocking {
        ...
        // register some handlers for state changes
        store.subscribe(this@StoreTest, NotProvidedState::class, true) { _ ->
    ...
    fun testUnprovidedStateDispatch() = runBlocking {
        ...
        // register some handlers for state changes
        store.subscribe(this@StoreTest, NotProvidedState::class, true) {
            // we should never get here because NotProvidedState isn't what's in the store.
            latch.countDown()
        }
    ...
    fun testUnsubscribeForAction() = runBlocking {
        ...
        // register some handlers for state changes
        val identifier = store.subscribe(this@StoreTest, MessagesState::class) {
    ...
    fun testAlternateSubscriptionSyntax() = runBlocking {
        ...
        // register some handlers for state changes
        store.subscribe(this@StoreTest, MessagesState::class) {
            print("hello")
        }
